...
def make_docx(plan) -> Path:
    ...
    doc.add_paragraph("LC = 3 LeetCode · HLD · LLD · Cloud · W = walk · H2O = water · F = food · C = chill · S = sleep")
    table(doc, ["Day", "Date", "LC", "HLD", "LLD", "Cloud", "W", "H2O", "F", "C", "S", "Notes"],
          [[d["day"], nice(d["date"])[:-5]] + ["☐"] * 9 + [""] for d in plan["days"]],
          widths=[1, 2.4, 0.9, 0.9, 0.9, 1.1, 0.8, 0.9, 0.8, 0.8, 0.8, 5.5])
...
def make_xlsx(plan) -> Path:
    ...
    tr = wb.active
    tr.title = "Tracker"
    cols = ["Day", "Date", "Type", "Theme", "LC 1", "LC 2", "LC 3", "HLD", "LLD", "Cloud",
            "Walk", "Water (L)", "Food", "Chill", "Sleep", "Done", "Of", "%", "Status", "Notes"]
    header(tr, cols, [6, 13, 15, 30, 6, 6, 6, 6, 6, 7, 6, 8, 6, 6, 6, 6, 5, 7, 12, 30])
    for i, d in enumerate(days, start=2):
        tr.append([d["day"], date.fromisoformat(d["date"]), d["type"], d["theme"]] + [""] * 11)
        tr.cell(row=i, column=2).number_format = "ddd d mmm"
        # done = ticks + water goal met
        tr.cell(row=i, column=16, value=f'=COUNTIF(E{i}:K{i},"{TICK}")+COUNTIF(M{i}:O{i},"{TICK}")+IF(N(L{i})>=2.5,1,0)')
        tr.cell(row=i, column=17, value=11)
        tr.cell(row=i, column=18, value=f"=P{i}/Q{i}")
        tr.cell(row=i, column=18).number_format = "0%"
        # Completed > Today > Partial (some ticks) > Upcoming (future, none) > Missed (past, none)
        tr.cell(row=i, column=19, value=(f'=IF(P{i}>=Q{i},"Completed",IF(B{i}=TODAY(),"Today",IF(P{i}>0,"Partial",'
    ...
    tr.conditional_formatting.add(f"E2:O{N + 1}", CellIsRule(operator="equal", formula=[f'"{TICK}"'],
    ...
    header(pl, ["Day", "Date", "Weekday", "Type", "Phase", "Week", "Theme", "DSA pattern", "Approach",
                ...
                "HLD read (lunch)", "HLD key points", "LLD in C# (9 pm)", "Cloud lab (9:30 pm)", "Checkpoint pass criteria"],
           [6, 12, 8, 14, 7, 6, 26, 20, 40, 30, 8, 30, 8, 30, 8, 36, 60, 40, 44, 50])
    for i, d in enumerate(days, start=2):
        lc = d["leetcode"]
        ...
        pl.row_dimensions[i].height = 62 if d["checkpoint"] else 48
    pl.auto_filter.ref = f"A1:T{N + 1}"
    ...
    last = N + 1
    rows = [
        ("Overall progress (tasks)", f"=SUM(Tracker!P2:P{last})/SUM(Tracker!Q2:Q{last})", "0%"),
        ("Days completed", f'=COUNTIF(Tracker!S2:S{last},"Completed")', "0"),
        ("Days partial", f'=COUNTIF(Tracker!S2:S{last},"Partial")', "0"),
        ("Days missed", f'=COUNTIF(Tracker!S2:S{last},"Missed")', "0"),
        ...
        ("HLD reads done", f'=COUNTIF(Tracker!H2:H{last},"{TICK}")', "0"),
        ("LLD builds done", f'=COUNTIF(Tracker!I2:I{last},"{TICK}")', "0"),
        ("Cloud labs done", f'=COUNTIF(Tracker!J2:J{last},"{TICK}")', "0"),
        ("Walk / exercise days", f'=COUNTIF(Tracker!K2:K{last},"{TICK}")', "0"),
        ("Water goal days (2.5 L+)", f'=COUNTIF(Tracker!L2:L{last},">=2.5")', "0"),
    ]
    r = 4
    ...
    for ph in plan["phases"]:
        ...
        c = db.cell(row=r, column=3, value=f"=SUM(Tracker!P{a}:P{b})/SUM(Tracker!Q{a}:Q{b})")
        c.number_format = "0%"
        db.cell(row=r, column=4, value=f'=COUNTIF(Tracker!S{a}:S{b},"Completed")')
